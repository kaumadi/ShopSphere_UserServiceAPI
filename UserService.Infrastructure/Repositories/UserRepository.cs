...
namespace UserService.Infrastructure.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task AddUserAsync(User user)
        {
            try
            {
                _logger.LogInformation("Fetching the user details for the provided email: {Email}", user.Email);

                await _context.Users.AddAsync(user);
                await _context.SaveChangesAsync();

                _logger.LogInformation("User successfully added with email: {Email}", user.Email);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "An error occurred while adding user with email: {Email}", user.Email);
                throw new ApplicationException("An error occurred while adding the user. Please try again later.");
            }
        }
    }
}
